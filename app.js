(() => {
  ...
  var { Download, RotateCcw, UploadCloud, Music, Trash2, Volume2, Clock, Loader2, Copy, AlertCircle, Activity, Server, Database, ShieldCheck, ImagePlus, Smartphone, Clapperboard, Type, Palette, Globe, MessageSquare, Monitor, Filter, Wand2, CloudRain, ChevronDown, Film, FileText, Layers, RefreshCw, Share2, Check, Link2, Newspaper, Scissors, ExternalLink, Eye } = ICONS;
  ...
  var isLocalhostAllowed = () => {
    if (typeof window === "undefined") return false;
    const isHttp = window.location.protocol === "http:";
    const isLocalHost = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1";
    return isHttp || isLocalHost;
  };
  var _linkedInServerUrl = "";
  var getLinkedInServerUrl = async () => {
  ...
  var shareToLinkedInAPI = async (text, imageBase64 = null, linkUrl = null, linkTitle = null, videoBase64 = null) => {
  ...
  var fetchWikimediaImages = async (query, limit = 3) => {
    try {
  ...
  var _isCanvasHttps = typeof window !== "undefined" && window.location?.protocol === "https:";
  var CORS_PROXIES = [
  ...
  var apiKey = typeof window !== "undefined" && (window.apiKey || window.__google_api_key) || SafeStorage.getItem("GEMINI_API_KEY") || "";
  var setGeminiApiKey = (key) => {
    apiKey = key || "";
  ...
  var sysEventBus = new EventBus();
  var _logBuffer = [];
  var addSystemLog = (text, type = "info") => {
  ...
  var exportWorkflowLog = (jobState) => {
  ...
  var getWPS = (lang) => ({ "en": 2.5, "es": 2.6, "fr": 2.4, "tr": 2.2, "ar": 2.2, "de": 2, "ru": 2 })[lang] || 2.2;
  var _getCurrentMonthYearTR = () => {
  ...
  var isFirebaseActive = initFirebase();
  var attemptSilentReauth = async () => {
    try {
      if (auth) {
  ...
  var ASSET_DB = "AINewsSaaS_Assets_v5";
  var STORE_MEDIA = "media_cache";
  var STORE_JOBS = "temporal_jobs";
  var LIB_STORE = "musicLib";
  var DIR_STORE = "dirHandles";
  var AssetManagerService = class {
    static async getDB() {
  ...
  var matchMusicToEmotion = (emotion, musicList) => {
  ...
  var _splitIntoStrips = (srcB64, stripCount) => {
  ...
  var _extractVideoFrame = (videoFile) => new Promise((resolve) => {
  ...
  var _ocrCall = async (imageB64, prompt, model, imgType, apiKey2) => {
  ...
  var _OCR_MODELS = AI_CONFIG.OCR_MODELS;
  var ocrWithFallback = async (b64Data, imgType, apiKey2, logPrefix = "G\xF6rsel") => {
  ...
  var _callGeminiAndParse = async (url, payload) => {
  ...
  var _base64ToBlob = (b64, mimeType = "audio/mpeg") => {
  ...
  var _createSilentOsc = (audioCtx, audioDest) => {
  ...
  var _loadScript = (src) => new Promise((resolve, reject) => {
  ...
  var convertWebMtoMP4 = async (inputBlob, onProgress) => {
  ...
  var LogicEngineService = class _LogicEngineService {
    ...
    static addRecentSonSoz(quote) {
      try {
        ...
        let list = _LogicEngineService.getRecentSonSozList();
    ...
    static _buildSonSozRule() {
      const recent = _LogicEngineService.getRecentSonSozList();
...
- T\xFCrk ve d\xFCnya edebiyat\u0131ndan, felsefesinden, sosyolojisinden, tarihinden ve b\xFCy\xFCk d\xFC\u015F\xFCn\xFCrlerinden (\xF6rne\u011Fin: Mevlana, Yunus Emre, Mustafa Kemal Atat\xFCrk, Platon, Aristoteles, Konf\xFC\xE7y\xFCs, Montaigne, Friedrich Nietzsche, Marcus Aurelius, Seneca, Fyodor Dostoyevski, Baruch Spinoza, Victor Hugo, \u0130bn Haldun, Niccol\xF2 Machiavelli, Naz\u0131m Hikmet, Cemil Meri\xE7, Mehmet Akif Ersoy, Franz Kafka, Lev Tolstoy, Albert Camus vb.) veya derin anlam ta\u015F\u0131yan \xF6zg\xFCn halk vecizelerinden tamamen farkl\u0131 bir bilge s\xF6z\xFC se\xE7 ve yaz.${recentText}`;
    ...
    static async analyzeContent(inputData, inputType, config) {
      ...
      if (config.tip === "guzel_soz") {
        return _LogicEngineService._buildGuzelSozScript(inputData, inputType, config);
      }
      if (config.tip === "iddia_analizi") {
        return _LogicEngineService._analyzeIddia(inputData, inputType, config);
      }
...
${timeConstraint}`;
      ...
      if (!isImageOutput) sonSozInstruction = _LogicEngineService._buildSonSozRule();
...
2. KES\u0130NT\u0130S\u0130Z SES VEYA V\u0130DEO OYNATMA ZORUNLULU\u011EU (H\u0130\xC7B\u0130R YER\u0130 KES\u0130LMEDEN):
   - Clickbait (ilk sahne) biter bitmez, olu\u015Fturaca\u011F\u0131n \u0130K\u0130NC\u0130 sahnede \`playOriginalMedia: true\` \xF6zelli\u011Fini ayarla. Bu sahnede \`spokenText\` ve \`topText\` bo\u015F b\u0131rak\u0131labilir, sadece orijinal medyan\u0131n kesintisiz oynat\u0131lmas\u0131 i\xE7in bir yer tutucu olacakt\u0131r.
   - Orijinal medya eksiksiz olarak dinletildikten/izletildikten HEMEN SONRA detayl\u0131 analiz ve do\u011Frulama a\u015Famas\u0131na (3. sahneye) ge\xE7ilecektir.
...
- Match the emotional tone` }] }],
  ...
  var MediaSynthesisService = class {
  ...
  var CustomSelect = ({ value, onChange, options, icon: Icon, className }) => {
  ...
  var GazeteCropModal = React.memo(({ src, name, onClose, onCrop }) => {
