...
//
// anti 1.0.23:
//   - Çift MP4 indirme engellendi: _globalLastAutoDownloadedJobId global kapsama taşındı ve duplikat a.click() çağrıları engellendi.
//   - Clickbait başlık ZORLA maks 3 kelime: _cleanMaxThreeWords tüm modlara (iddia analizi, haber, gazete, prompt) uygulandı.
//
// anti 1.0.22:
//   - İddia Analizi Modu: Yüklenen video/ses/görsel sonuna kadar kesintisiz oynatılır, hemen ardından Türkiye gerçekleri ile eleştirel, tarafsız ve kaynaklı analiz başlar.
//
// anti 1.0.19:
//   - Zorla sıfır boşluk: Sahneler arası ses bitişinde anında sonraki sahneye geçiş (ab.duration & audioEnded listener)
//   - Güzel söz ve haber modlarında minDur/cushion yapay beklemeleri tamamen kaldırıldı
//
// black_2.3:
...
const ICONS = {
  ...
  Newspaper: ({size=16, ...p}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><line x1="18" y1="14" x2="18" y2="18"/><line x1="15" y1="18" x2="21" y2="18"/><line x1="6" y1="7" x2="10" y2="7"/><line x1="6" y1="11" x2="10" y2="11"/><line x1="6" y1="15" x2="10" y2="15"/></svg>,
...
const convertWebMtoMP4 = async (inputBlob, onProgress) => {
  ...
  // 0. ÖNCELİK: Zaten MP4 ise dönüştürme yapma
  if (inputBlob.type && inputBlob.type.includes('mp4')) {
    addSystemLog('Video zaten native MP4 formatında, dönüştürme gerekmedi.', 'info');
    return inputBlob;
  }

  addSystemLog('Instagram uyumlu 30 FPS MP4 dönüştürme başlatılıyor...', 'info');

  // 1. ÖNCELİK: Yerel Python Sunucusu (Sistem ffmpeg — 100x hızlı)
  try {
    const formData = new FormData();
    ...
    const resp = await fetch(proxyUrl, { method: 'POST', body: formData, signal: AbortSignal.timeout(3000) });
    if (resp.ok) {
    ...
    addSystemLog('Yerel ffmpeg servisi aktif değil (WASM kontrol ediliyor)...', 'info');
  }

  // 2. ÖNCELİK: Tarayıcı İçi ffmpeg.wasm Fallback (SharedArrayBuffer Varsa)
  if (typeof SharedArrayBuffer === 'undefined') {
    addSystemLog('SharedArrayBuffer tarayıcıda devre dışı (WASM atlandı). Video doğrudan paketleniyor.', 'info');
    return inputBlob;
  }
  ...
  try {
    ...
    addSystemLog('✅ ffmpeg.wasm ile 30 FPS MP4 dönüşümü tamamlandı!', 'success');
    return mp4Blob;
  } catch (err) {
    addSystemLog('MP4 dönüştürme atlandı, orijinal video kullanılacak: ' + err.message, 'warn');
    return inputBlob;
  }
};
...
Dönüş ZORUNLU olarak JSON formatında olmalı.`;
  ...
  let cleaned = text.replace(/.*(video başlığı|ekranda yer alan|kimlik ve görev bilgisi|görsel betimlemesi|prompt).*/gi, '')
                    .replace(/\d{1,2}\s+[A-Za-zĞÜŞİÖÇğüşıöç]+\s+\d{4}/g, '')
  ...
  // KESİNLİKLE MAKSİMUM 3 KELİME ZORLAMASI (4 VEYA DAHA FAZLA KELİME KESİNLİKLE OLAMAZ)
...
ZORUNLU İDDİA ANALİZİ AKIŞI VE SENARYO KURALLARI:

1. SLAYT 1 - VURUCU HOOK (5sn Clickbait İddia Başlığı):
   - Araştırılacak iddiayı / konuyu duyuran, merak uyandıran sansasyonel ama doğru ilk başlık.

2. SLAYT 2 - KESİNTİSİZ ORİJİNAL MEDYA OYNATIMI (HİÇBİR YERİ KESİLMEDEN):
   - İkinci sahnede mutlaka playOriginalMedia: true özelliğini ayarla.
   - Bu sahnede yüklenen video veya ses SONUNA KADAR KESİNTİSİZ ÇALINIR / GÖSTERİLİR.
   - spokenText ve topText BOŞ ("") bırakılır ("ORİJİNAL KAYIT" etiketi yer alacaktır).

3. SLAYT 3 VE DEVAMI - ELEŞTİREL VE TARAFSIZ TÜRKİYE GERÇEKLERİ ANALİZİ (HEMEN MEDYADAN SONRA):
   - Medya eksiksiz dinletildikten/izletildikten HEMEN SONRAKİ SAYFALARDA Türkiye'deki o konu ile ilgili GERÇEKTE NE OLDUĞUNU anlayan, bulan ve açıklayan ELEŞTİREL, TARAFSIZ VE NESNEL ANALİZ başlar.
   - Konu Türkiye bağlamında ele alınır: "Türkiye'de bu konuda gerçekte ne yaşanıyor?" sorusu yanıtlanır.
   - Resmi devlet verileri (TÜİK, TCMB, TÜRK-İŞ, DİSK-AR, Hazine, Adalet Bakanlığı, mahkeme kararları, reel piyasa kurları ve alım gücü) ışığında değerlendirilir.
   - 2002 yılı ile günümüz kıyaslaması yapılır ("2002'de X verisi şu kadardı, günümüz ${_curMonthYear} itibarıyla şu oldu").
   - İddiadaki doğru, yanlış ve eksik yönler net, cesur ama tamamen TARAFSIZ VE NESNEL bir üslupla ortaya konur.

4. MUTLAKA YAZILACAK RESMİ KAYNAK VE LİNK ZORUNLULUĞU:
   - Değerlendirmelerinde resmi kurum verilerini (TÜİK, TCMB, TÜRK-İŞ, Hazine, Resmi Gazete vb.) kullan.
   - Sahnelerde ve çıkarılan kanıtlarda MUTLAKA RESMİ KAYNAK ADINI, TARİHİNİ (${_curMonthYear}) VE VARSA WEB ADRESİNİ (URL) açıkça yaz.

5. GÜNCEL RESMİ EKONOMİ VE TARİHSEL BAZ VERİLERİ (${_curMonthYear}):
${buildEconomicDataBlock()}

6. GÖREV VE VİDEO SENARYO AKIŞI (videoSlides):
   - Slayt 1 (Hook) -> Slayt 2 (Kesintisiz Orijinal Medya) -> Slayt 3+ (Türkiye Gerçekleri & Eleştirel Tarafsız Analiz) -> Son Slayt (KAYNAKLAR).${LogicEngineService._buildSonSozRule()}

7. META-PROMPT VE BETİMLEME SIZINTISI YASAKTIR:
   - spokenText İÇİNE KESİNLİKLE GÖRSEL TANIMLAMALARI YA DA PROMPT CÜMLELERİ YAZMA!
   - spokenText SADECE İZLEYİCİYE HİTABEN OKUNACAK DOĞAL DIŞ SES CÜMLELERİNDEN OLUŞMALIDIR.
...
Dönüş ZORUNLU olarak geçerli JSON formatında olmalıdır.`;
  ...
  static async _buildGuzelSozScript(inputData, inputType, config) {
    ...
    // ── Çok dilli çeviri: FR → DE → TR ─────────────────────────────────────
    addSystemLog('Çok dilli çeviri: FR / DE / TR...', 'info');
    let textFR = quoteText, textDE = quoteText, textTR = quoteText;
    try {
      const transUrl = `https://generativelanguage.googleapis.com/v1beta/models/${AI_CONFIG.GEMINI_MODEL}:generateContent?key=${apiKey}`;
      const transPayload = {
        contents: [{ parts: [{ text: `Translate the following quote into French, German, and Turkish. Keep the poetic tone. Return ONLY a JSON object with keys "fr", "de", "tr" and nothing else.\n\nQuote: "${quoteText}"` }] }],
        generationConfig: { temperature: 0.5, maxOutputTokens: 300 }
      };
      const transR = await NetworkUtils.fetchWithRetry(transUrl, { method: 'POST', body: JSON.stringify(transPayload) });
      if (transR) {
        const transData = await transR.json();
        const transRaw = transData.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || '{}';
        const jsonMatch = transRaw.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
          const translations = JSON.parse(jsonMatch[0]);
          if (translations.fr) { textFR = translations.fr; addSystemLog(`FR: ${textFR.substring(0, 40)}...`, 'success'); }
          if (translations.de) { textDE = translations.de; addSystemLog(`DE: ${textDE.substring(0, 40)}...`, 'success'); }
          if (translations.tr) { textTR = translations.tr; addSystemLog(`TR: ${textTR.substring(0, 40)}...`, 'success'); }
        }
      }
    } catch (e) { addSystemLog('Çeviri hatası, orijinal metin kullanılıyor: ' + e.message, 'warn'); }

    // Her dil kendi sahnesine atanıyor: 0=FR, 1=DE, 2=TR
    const langTexts = [textFR, textDE, textTR];
    const langCodes = ['fr', 'de', 'tr'];
    const langLabels = ['🇫🇷 FR', '🇩🇪 DE', '🇹🇷 TR'];

    return {
      isContentUnreadable: false,
      videoSlides: sceneDescriptions.map((desc, i) => ({
            topText: langTexts[i] || quoteText,
            spokenText: langTexts[i] || quoteText,
            imagePrompts: [desc],
            _langCode: langCodes[i] || 'tr',
            _langLabel: langLabels[i] || '🇹🇷 TR'
          })),
      thumbnailText: textTR.length > 120 ? textTR.substring(0, 120) + '...' : textTR,
      sonSoz: "",
      lastQuote: quoteText,
  ...
  static async generateAudio(text, voice) {
    ...
    const RenderWorkerService = {
      ...
      renderGuzelSoz: async (jobData, canvasElement, w, h, cx, fontFamily, preferences) => {
        addSystemLog('Güzel söz render başlıyor (çok dilli: FR→DE→TR)...', 'info');
        const slides = jobData.script.videoSlides || [];
        const isMultilang = jobData.script._isMultilang && slides.length >= 3;
        const FPS = 30;
        ...
        // ── Her sahnenin ses süresini önceden hesapla ─────────────────────
        const sceneDurations = []; // sn cinsinden her sahne süresi
        const sceneAudioBufs = []; // decode edilmiş AudioBuffer'lar
        for (let si = 0; si < sceneCount; si++) {
          const audioData = jobData.assets.audio[si];
          const slideText = slides[si]?.spokenText || "";
          const wordCount = slideText.split(/\s+/).filter(Boolean).length;
          const minDur = Math.max(4.0, (wordCount / 2.2) + 2.0);
          if (audioData?.wavBuffer) {
            try {
              let bufCopy;
              if (audioData.wavBuffer instanceof ArrayBuffer) bufCopy = audioData.wavBuffer.slice(0);
              else if (audioData.wavBuffer.buffer instanceof ArrayBuffer) bufCopy = audioData.wavBuffer.buffer.slice(0);
              else bufCopy = audioData.wavBuffer;
              const ab = await audioCtx.decodeAudioData(bufCopy);
              sceneAudioBufs.push(ab);
              const dur = Math.min(Math.max(ab.duration + 0.5, minDur), maxAllowedDur);
              sceneDurations.push(dur);
              addSystemLog(`Sahne ${si+1} (${slides[si]?._langCode?.toUpperCase()}): ${ab.duration.toFixed(1)}sn → ${dur.toFixed(1)}sn`, 'info');
            } catch(e) { sceneAudioBufs.push(null); sceneDurations.push(minDur); addSystemLog(`Sahne ${si+1} ses decode hatası.`, 'warn'); }
          } else { sceneAudioBufs.push(null); sceneDurations.push(minDur); }
        }

        const totalDuration = Math.min(sceneDurations.reduce((a, b) => a + b, 0) + 0.5, maxAllowedDur * sceneCount);
        const totalFrames = Math.round(totalDuration * FPS);
        addSystemLog(`Toplam süre: ${totalDuration.toFixed(1)}sn (${sceneCount} sahne: ${sceneDurations.map(d=>d.toFixed(1)).join('+')})`, 'info');

        // ── BGM: tüm video boyunca çal ────────────────────────────────────
        ...
        if (ambientSound === 'none') {
          try {
            ...
            if (allMusic.length > 0) { ambientSound = allMusic[0].id; addSystemLog(`Müzik otomatik seçildi: ${allMusic[0].name}`, 'info'); }
          ...
          if (ambientTypes.includes(ambientSound)) {
            ...
            try {
              ...
              if (track?.data) {
                ...
                masterGain = audioCtx.createGain(); masterGain.gain.value = preferences?.backgroundMusicVolume ?? RENDER_CONFIG.BGM_VOLUME;
        ...
        // ── Görseller yükle ───────────────────────────────────────────────
        const rawImages = jobData.assets.images.filter(img => img);
        const loadedImages = [];
        ...
        // ── ÇOKLU SAHNE RENDER: Her sahne kendi sesini çalar, ses bitince geçer ──
        let globalFrame = 0;
        const crossfadeFrames = Math.floor(FPS * 0.5);
        const kenBurnsDirs = [0, 1, 2].map(() => Math.floor(Math.random() * 4));

        for (let si = 0; si < sceneCount; si++) {
          const sceneText = slides[si]?.spokenText || "";
          const langLabel = slides[si]?._langLabel || '';
          const sceneDur = sceneDurations[si];
          const sceneFrames = Math.round(sceneDur * FPS);
          const imgIndex = Math.min(si, loadedImages.length - 1);
          const nextImgIndex = Math.min(si + 1, loadedImages.length - 1);
          const kenBurnsDir = kenBurnsDirs[si];

          // Bu sahnenin sesini başlat
          const ab = sceneAudioBufs[si];
          ...
          if (ab) {
            ...
            voiceSource.onended = () => { voiceEnded = true; };
            voiceSource.start(0);
            addSystemLog(`Sahne ${si+1} sesi başladı (${langLabel}) — vol:${safeVol.toFixed(2)}.`, 'info');
          }

          for (let f = 0; f < sceneFrames; f++, globalFrame++) {
            // ZORLA SIFIR BOŞLUK: Ses bittiğinde anında sonraki sahneye geç
            if (typeof voiceEnded !== 'undefined' && voiceEnded && f >= Math.min(8, sceneFrames - 1)) {
              addSystemLog(`Güzel Söz Sahne ${si+1} ses bitti, sonraki sahneye anında geçiliyor.`, 'info');
            ...
            if (loadedImages[imgIndex]) {
              ...
              const imgR = img.width / img.height; const canR = w / h;
              let sx, sy, sw, sh;
            ...
            // Crossfade sonuna doğru bir sonraki görsele geç
            if (f > sceneFrames - crossfadeFrames && nextImgIndex !== imgIndex && loadedImages[nextImgIndex]) {
              const fadeP = (f - (sceneFrames - crossfadeFrames)) / crossfadeFrames;
              ctx.globalAlpha = Math.min(1, fadeP);
              const ni = loadedImages[nextImgIndex];
              const niR = ni.width / ni.height; const canR2 = w / h;
              let sx2, sy2, sw2, sh2;
              if (niR > canR2) { sh2 = ni.height; sw2 = sh2 * canR2; sx2 = (ni.width - sw2)/2; sy2 = 0; }
              else { sw2 = ni.width; sh2 = sw2 / canR2; sx2 = 0; sy2 = (ni.height - sh2)/2; }
              ctx.drawImage(ni, sx2, sy2, sw2, sh2, 0, 0, w, h);
              ctx.globalAlpha = 1;
            }

            // Gradient overlay
            const ov = ctx.createLinearGradient(0, 0, 0, h);
            ov.addColorStop(0, 'rgba(0,0,0,0.55)'); ov.addColorStop(0.35, 'rgba(0,0,0,0.1)');
            ov.addColorStop(0.65, 'rgba(0,0,0,0.1)'); ov.addColorStop(1, 'rgba(0,0,0,0.65)');
            ctx.fillStyle = ov; ctx.fillRect(0, 0, w, h);

            // Dil etiketi — sağ üst köşe
            if (langLabel) {
              const fadeIn2 = Math.min(1, (f / FPS) / 0.5);
              ctx.save();
              ctx.globalAlpha = fadeIn2 * 0.9;
              ctx.font = `bold ${w > 800 ? 22 : 16}px ${fontFamily}`;
              ctx.textAlign = 'right'; ctx.textBaseline = 'top';
              const lx = w - (w > 800 ? 32 : 20);
              const ly = w > 800 ? 28 : 18;
              ctx.lineWidth = 4; ctx.strokeStyle = 'rgba(0,0,0,0.7)'; ctx.lineJoin = 'round';
              ctx.strokeText(langLabel, lx, ly);
              ctx.fillStyle = '#FFD700'; ctx.fillText(langLabel, lx, ly);
              ctx.restore();
            }

            // Alıntı metni — ortada
            const fadeIn = Math.min(1, (f / FPS) / 0.6);
            ctx.save();
            ctx.globalAlpha = fadeIn;
            const maxLines = Math.floor((h * 0.68) / (36 * 1.5));
            let fitFontSize = w > 800 ? 46 : 34;
            ctx.font = `bold ${fitFontSize}px ${fontFamily}`;
            let fitLines = RenderWorkerService.wrapText(ctx, sceneText, w * 0.82);
            let lh = fitFontSize * 1.5;
            let totalH = fitLines.length * lh;
            while ((totalH > h * 0.68 || fitLines.length > maxLines) && fitFontSize > 16) {
              fitFontSize -= 2;
              ctx.font = `bold ${fitFontSize}px ${fontFamily}`;
              fitLines = RenderWorkerService.wrapText(ctx, sceneText, w * 0.82);
              lh = fitFontSize * 1.5;
              totalH = fitLines.length * lh;
            }
            ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
            const startY = (h - totalH) / 2;
            fitLines.forEach((line, li) => {
              const y = startY + (li * lh) + (lh / 2);
              ctx.lineWidth = 5; ctx.strokeStyle = '#000'; ctx.lineJoin = 'round';
              ctx.strokeText(line, cx, y);
              ctx.fillStyle = '#FFF'; ctx.fillText(line, cx, y);
            });
            ctx.restore();

            if (videoTrack?.requestFrame) videoTrack.requestFrame();
            if (f % 30 === 0) {
              const totalProg = (si * (1/sceneCount)) + (progress / sceneCount);
              sysEventBus.emit('PROGRESS', { step: 'RENDER', percent: Math.min(90, 30 + totalProg * 60), text: `Sahne ${si+1}/${sceneCount} (${langLabel}) — ${elapsed.toFixed(1)}sn` });
            }
            await nextFrame();
          }

          // Sahne sesi durdur
          if (voiceSource) { try { voiceSource.stop(); } catch(e) { ErrorHandler.silent(e); } }
          addSystemLog(`Sahne ${si+1} tamamlandı.`, 'success');
        }
      ...
      executeRender: async (jobData, canvasElement, preferences) => {
        ...
        const playAudio = async (audioData, requestedDuration = null, fallbackText = "") => {
          ...
          // ZORLA SIFIR BOŞLUK: Ses verisi çözüldüyse exact ses süresi tek kaynaktır (tahmin süresi ile ezilmez)
          let totalDur = (audioData?.wavBuffer && scaledExactDur > 0) ? scaledExactDur : (requestedDuration !== null ? requestedDuration : scaledExactDur);
        ...
        try {
          ...
          const streamingEnabled = false; // Bellek içi Blob kaydı — gereksiz dosya seçici penceresi ve çift indirme engellendi
                ...
                try {
                  ...
                  const clickbaitPayload = {
                    contents: [{ parts: [{ text: `Bu haber başlıklarından en etkileyici, clickbait bir tek başlık oluştur (maksimum 10 kelime, büyük harfler, sansasyonel):
                          ...
                          if (cr) {
                            ...
                            if (clickbaitText) {
                              this.state.script.thumbnailText = _cleanMaxThreeWords(clickbaitText);
                        ...
                        if (!this.state.script.thumbnailText || this.state.script.thumbnailText.length < 5) {
                          ...
                          this.state.script.thumbnailText = _cleanMaxThreeWords(longest);
                  ...
                  if (this.state.status === 'GENERATING_ASSETS') {
                    ...
                    if (this.state.script._isGuzelSoz) {
                      ...
                      // Çok dilli: her sahne için ayrı ses üret (FR→DE→TR)
                      const slideCount2 = this.state.script._sceneCount || 3;
                      for (let ai = 0; ai < slideCount2; ai++) {
                        if (!this.state.assets.audio[ai]) {
                          const slideText = this.state.script.videoSlides[ai]?.spokenText || "";
                          const langCode = this.state.script.videoSlides[ai]?._langCode || 'tr';
                          if (slideText) {
                            try {
                              addSystemLog(`Ses ${ai + 1}/${slideCount2} üretiliyor (${langCode.toUpperCase()})...`, 'info');
                              this.state.assets.audio[ai] = await MediaSynthesisService.generateAudio(
                                slideText,
                                this.state.preferences.narratorVoice
                              );
                              addSystemLog(`Ses ${ai + 1} tamamlandı.`, 'success');
                            } catch(e) { addSystemLog(`Ses ${ai + 1} hatası: ${e.message}`, 'warn'); }
                          }
                        }
                      }
                      ...
                      if (allMusic.length > 0) {
                        ...
                        addSystemLog(`Müzik belirlendi: ${chosenTrack.name} (Seçili Müzik Korundu)`, 'success');
...
let _globalLastAutoDownloadedJobId = null;

              const autoSaveVideo = async (videoUrl, title, videoFormat, jobId = null) => {
                if (!videoUrl || !videoUrl.startsWith('blob:')) {
                  addSystemLog('Geçersiz video URL, kaydetme atlandı.', 'warn');
                  return;
                }
                if (jobId && _globalLastAutoDownloadedJobId === jobId) {
                  return; // Duplikat indirme engeli
                }
                if (jobId) _globalLastAutoDownloadedJobId = jobId;
                if (!videoUrl || !videoUrl.startsWith('blob:')) {
                  addSystemLog('Geçersiz video URL, kaydetme atlandı.', 'warn');
                ...
                if (jobId && _lastAutoDownloadedJobId === jobId) {
                  return; // Aynı iş için duplikat indirme engeli
                }
                if (jobId) _lastAutoDownloadedJobId = jobId;
                ...
                try {
                  ...
                  const fileName = `${safeName || 'video'}${ext}`;
              ...
              const uploadMediaToCloud = async (blobOrUrl, fileName = 'video.mp4') => {
                try {
                  ...
                  // Servis 1: tmpfiles.org (Doğrudan MP4 linki sunar, %100 NATIVE CORS UYUMLU)
                  try {
                  ...
                  try {
                    ...
                    if (r2.ok) {
                      ...
                      if (pubUrl && pubUrl.startsWith('http')) {
                        addSystemLog(`✓ Video/Medya buluta başarıyla yüklendi (catbox): ${pubUrl}`, 'success');
                        return pubUrl;
                      }
                    }
              ...
              useEffect(() => {
                  ...
                  const onWorkflowState = (data) => {
                    ...
                    if (data.status === 'COMPLETED') {
                      ...
                      autoSaveVideo(data.job.videoUrl, data.job.script?.thumbnailText || 'video', data.job.config?.videoFormat, data.job.jobId);
                  ...
                  // React 18 Strict Mode double-mount koruması: cleanup ile listener'ları kaldır
                  return () => {
                  ...
                  const loadLocalMusic = async () => {
                    try {
                      ...
                      if (allMusic.length > 0) {
                        ...
                        // Passive music check: quiet mode
                      }
              ...
              const NEWSPAPER_DIRECT_CONFIG = [
                ...
                { name: 'Hürriyet', type: 'static_gzt', url: 'https://img.piri.net/piri/upload/3/2026/7/28/2c109f9f-1b53de80492b43b7ad063d2c7304befe.jpg' },
              ...
              // 30 Ulusal Gazetenin ilk sayfalarını %100 doğrudan ve CORS-free getiren fonksiyon
              ...
              const matchOrFormatGazeteName = (rawName) => {
                ...
                const GAZETE_LIST = [
                  ...
                  'Yeni Birlik', 'Yeni Mesaj', 'Yeni Şafak'
                ];
                const lower = rawName.toLowerCase();
                // 1. Resmi 30 ulusal gazete listesinden tam eşleştirme
              ...
              return (
                ...
                {activeTab !== 'media' && activeTab !== 'gazete' && (
                    <textarea value={textInput} onChange={e => setTextInput(e.target.value)} placeholder={config.tip === 'iddia_analizi' ? (activeTab === 'url' ? "Analiz edilecek iddia / haber linkini yapıştırın..." : "İddiayı, haberi veya araştırılacak konuyu yazın (veya medya yükleyin)...") : config.tip === 'guzel_soz' ? (activeTab === 'url' ? "Söz linkini yapıştırın..." : "Güzel sözü veya alıntıyı yazın...") : (activeTab === 'url' ? "Haber linkini yapıştırın..." : "Haberi yazın veya araştırılacak gündemi verin...")} className={`w-full h-20 bg-black/30 border rounded-xl p-3 text-sm outline-none mb-3 text-slate-200 resize-none transition-all relative z-0 ${activeTab === 'prompt' ? 'border-fuchsia-500/50 focus:border-fuchsia-500' : 'border-slate-800 focus:border-indigo-500'}`} />
                  )}
