...
//   - Uygulama tamamen cevimdisi/standalone calisir.
// ============================================================================
...
// ============================================================================
const APP_VERSION = {
  major: 1, minor: 32, hotfix: 'H1.32',
  toString() { return `anti ${this.major}.0.${this.minor}`; },
  toBadge() { return `${this.toString()} • One-Page`; }
};

const RENDER_CONFIG = {
  FPS: 30, TIMER_WORKER_INTERVAL_MS: 33, WINDOW_SIZE: 5,
...
const setGeminiApiKey = (key) => { apiKey = key || ""; SafeStorage.setItem('GEMINI_API_KEY', apiKey); };
...
const exportWorkflowLog = (jobState) => { const lines = ['=== AI News Studio Workflow Log ===', `Tarih: ${new Date().toLocaleString('tr-TR')}`, `Versiyon: ${APP_VERSION.toString()}`, '']; lines.push('--- Sistem Logları ---'); for (const e of _logBuffer) lines.push(`[${e.timestamp}] [${e.type.toUpperCase()}] ${e.text}`); lines.push(''); lines.push('--- Workflow State ---'); lines.push(`Job ID: ${jobState?.jobId || 'N/A'}`); lines.push(`Status: ${jobState?.status || 'N/A'}`); lines.push(`Slides: ${jobState?.script?.videoSlides?.length || 0}`); lines.push(`ImageBlocks: ${jobState?.script?.imageBlocks?.length || 0}`); lines.push(`Images generated: ${jobState?.assets?.images?.filter(Boolean).length || 0}/${jobState?.assets?.images?.length || 0}`); lines.push(`Audio generated: ${jobState?.assets?.audio?.filter(Boolean).length || 0}/${jobState?.assets?.audio?.length || 0}`); lines.push(`Config: ${JSON.stringify(jobState?.config || {}, null, 2)}`); lines.push(''); lines.push('--- Slide Details ---'); for (const [i, s] of (jobState?.script?.videoSlides || []).entries()) { lines.push(`S${i + 1}: "${(s.spokenText || '').substring(0, 80)}..." img=${!!jobState?.assets?.images?.[i]} aud=${!!jobState?.assets?.audio?.[i]}`); } const blob = new Blob([lines.join('\n')], { type: 'text/plain' }); const a = document.createElement('a'); a.href = ObjectURLManager.create(blob); a.download = `log_${Date.now()}.txt`; a.click(); };
window.exportWorkflowLog = exportWorkflowLog;

const getWPS = (lang) => ({ 'en': 2.5, 'es': 2.6, 'fr': 2.4, 'tr': 2.2, 'ar': 2.2, 'de': 2.0, 'ru': 2.0 }[lang] || 2.2);
const _getCurrentMonthYearTR = () => { const now = new Date(); const months = ['Ocak', 'Şubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık']; return months[now.getMonth()] + ' ' + now.getFullYear(); };
const _getCurrentDateTR = () => { const now = new Date(); const months = ['Ocak', 'Şubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık']; return now.getDate() + ' ' + months[now.getMonth()] + ' ' + now.getFullYear(); };
const getDurationBounds = (dur) => { if (dur === '15') return { min: 15.0, max: 25.0 }; if (dur === '30') return { min: 28.0, max: 35.0 }; if (dur === '60') return { min: 56.0, max: 64.0 }; if (dur === '90') return { min: 85.0, max: 95.0 }; return { min: 0.0, max: 9999.0 }; };
  ...
  fetchWithRetry: async (url, options, retries = 5) => {
    ...
    for (let i = 0; i < retries; i++) {
      ...
      try {
        ...
        if (res.status === 401) { addSystemLog('Yetkilendirme hatası (401) — Gemini API Key\'inizi kontrol edin.', 'error'); throw new Error('HTTP_FAIL_401'); }
...
ILACAK YAP!.mp3", "url": "https://docs.google.com/uc?export=download&id=1bpZEyhCu88tZcene22Vd8nKr_dH5GfJg"}, {"id": "gdrive_1sj_UsiSAI07J2AmYy6vz1FPZSdRJ4T_X", "driveId": "1sj_UsiSAI07J2AmYy6vz1FPZSdRJ4T_X", "name": "ölürem türkiyem.mp3", "url": "https://docs.google.com/uc?export=download&id=1sj_UsiSAI07J2AmYy6vz1FPZSdRJ4T_X"}];
...
class AssetManagerService {
  ...
  static async getAllMusicFromLib() {
    try {
      ...
      const localList = await new Promise(r => req.onsuccess = () => r(req.result || []));
      const localMap = new Map(localList.map(m => [m.id, m]));
      const merged = [...localList];
      for (const cloudTrack of GDRIVE_CLOUD_MUSIC_CATALOG) {
        if (!localMap.has(cloudTrack.id)) {
          merged.push({ id: cloudTrack.id, name: cloudTrack.name, isCloud: true, driveId: cloudTrack.driveId, url: cloudTrack.url });
        }
      }
      return merged;
    } catch (e) { return GDRIVE_CLOUD_MUSIC_CATALOG || []; }
  }
  static async getMusicFromLib(id) {
    try {
      ...
      const local = await new Promise(r => req.onsuccess = () => r(req.result || null));
      if (local && local.data) return local;
      const cloudTrack = GDRIVE_CLOUD_MUSIC_CATALOG.find(t => t.id === id || t.driveId === id);
      if (cloudTrack) {
        addSystemLog(`Cloud müzik indiriliyor: ${cloudTrack.name}...`, 'info');
        const driveUrl = `https://docs.google.com/uc?export=download&id=${cloudTrack.driveId}`;
        const proxyUrls = [
          driveUrl,
          `https://api.allorigins.win/raw?url=${encodeURIComponent(driveUrl)}`,
          `https://corsproxy.io/?url=${encodeURIComponent(driveUrl)}`
        ];
        for (const u of proxyUrls) {
          try {
            ...
            if (resp.ok) {
              ...
              const savedObj = { id: cloudTrack.id, name: cloudTrack.name, data: b64, isCloud: true };
              await this.saveMusicToLib(savedObj);
              addSystemLog(`✓ Müzik önbelleğe alındı: ${cloudTrack.name}`, 'success');
              return savedObj;
            }
          } catch(err) {}
        }
...
// === WebM → MP4 DÖNÜŞÜM (ffmpeg.wasm @0.11) ===
...
class LogicEngineService {
  ...
  static async analyzeContent(inputData, inputType, config) {
    ...
    const sysPrompt = `Sen TikTok ve Instagram Reels için viral içerikler üreten profesyonel bir içerik üreticisisin. Karakterin: Zeki, gerçekleri söyleyen, 20 yaşında dertli bir genç.\n\nSENARYOYU ${isImageOutput ? 1 : sceneCount} SAHNE olacak şekilde böl!\nToplam konuşma metni ${words} kelime aralığında olmalıdır.\n\nDİL KURALI: ${langInstruction}\n${styleInstruction}\n${dynamicRules}\n\nEKONOMI KURALLARI (ekonomi haberi ise): Turkce karakter kullan, TL para birimi, sayi bicimi 85.450 TL, kaynak belirt (TUIK, TCMB, TURK-IS), aclik/yoksulluk siniri guncel olsun. Bilgi kartlari olustur: ENFLASYON %XX, ACLIK SINIRI XX.XXX TL.\n\nGAZETE BAŞLIKLARI: Görseldeki TÜM haber başlıklarını çıkar. Her başlık için 'baslik', 'aciklama', 'x', 'y', 'w', 'h' ver. En az 1, en fazla 15 başlık. Reklam, bulmaca, ilan HARİÇ.\n\nKAPAK VE VİRAL HOOK KURALLARI (MAKSİMUM 3 KELİME KURALI):\n1. 'thumbnailText': KESİNLİKLE MAKSİMUM 3 KELİME ultra-tıklama odaklı başlık.\n2. 'topText': KESİNLİKLE MAKSİMUM 3 KELİME.\n3. 'thumbnailImagePrompt': Ultra-dramatik, sinematik, 8K HDR İngilizce kapak görsel promptu.\n\nTAM 60 SANİYELİK PROFESYONEL VİDEO AKIŞ KURALLARI (TOPLAM 60 SANİYE):
1. 1. SAHNE (CLICKBAIT HOOK - 4-5 sn / ~10-12 kelime): 'thumbnailText' (KESİNLİKLE MAKSİMUM 3 KELİME) + merak uyandıran, izleyiciyi ekrana kilitleyen dramatik kanca açılış cümlesi. 'thumbnailImagePrompt' ultra-sinematik kapak görseli.
2. 2. - 6. SAHNELER (GAZETE MANŞET & DETAY HABER OKUMA - 45-48 sn / ~95-105 kelime): Gazetedeki ana manşeti ve en can alıcı 2-3 haber detayını 5N1K kuralıyla, akıcı, tempolu, sürükleyici bir haber spikeri üslubuyla oku. Gazetedeki başlıkları ve açıklamaları detaylandır. 'imagePrompts' dizisini BOŞ [] BIRAK (görüntüde gazete ilk sayfası kalır).
3. 7. SAHNE (SON SÖZ - 4-5 sn / ~10-12 kelime): Haberin ana fikrine cuk oturan çok vurucu bir ATASÖZÜ veya ÖZLÜ SÖZ ('sonSoz').
4. 8. SAHNE (OUTRO - 3-4 sn / ~6-8 kelime): 'lastQuote' abone olma ve takip etme çağrısı.
TOPLAM METİN: 120-135 kelime aralığında olmalıdır.\n- SIFIR HALÜSİNASYON: Okuyamadıysan 'isContentUnreadable' true yap.\n- ATATÜRK HASSASİYETİ: 'Atatürk' geçerse "Mustafa Kemal Atatürk, highly detailed, respectful portrait" ekle!${sonSozInstruction}\nDönüş ZORUNLU olarak JSON formatında olmalı.`;
    ...
    const _cleanMaxThreeWords = (text) => {
      ...
      if (words.length > 3) return words.slice(0, 3).join(' ').toUpperCase() + '!';
      return words.join(' ').toUpperCase() + '!';
    };
    if (parsedData.thumbnailText) parsedData.thumbnailText = _cleanMaxThreeWords(parsedData.thumbnailText);
        ...
        responseSchema: {
          ...
          properties: {
            ...
            iddialar: { type: 'ARRAY', items: { type: 'OBJECT', properties: {
              iddia: { type: 'STRING' }, durum: { type: 'STRING' }, guvenSkoru: { type: 'NUMBER' },
              analiz: { type: 'STRING' },
  ...
  static async _translateQuoteMultilang(quoteText) {
    ...
    try {
      ...
      if (r && r.ok) {
        ...
        return { fr: parsed.fr || quoteText, de: parsed.de || quoteText, tr: parsed.tr || quoteText };
      }
    } catch (e) { addSystemLog('Çeviri hatası: ' + e.message + ' — orijinal metin kullanılacak', 'warn'); }
  ...
  static async _buildGuzelSozScript(inputData, inputType, config) {
    ...
    if (isAtaturkRelated) {
      ...
      else addSystemLog('Wikimedia\'dan görsel bulunamadı — AI görseller kullanılacak.', 'warn');
    }
    ...
    return {
      ...
      sonSoz: "", lastQuote: quoteText, thumbnailImagePrompt: sceneDescriptions[0] || "",
...
class MediaSynthesisService {
  ...
  static async generateImage(prompt, imageStyle = 'cinematic', resolution = '4K', isGuzelSoz = false, emotion = 'notr', quoteText = '') {
    ...
    if (isGuzelSoz && quoteText) { addSystemLog('Quote uyumlu fallback görsel üretiliyor...', 'warn'); return this.generateQuoteFallback(quoteText, emotion); }
...
const RenderWorkerService = {
  ...
  renderGuzelSoz: async (jobData, canvasElement, w, h, cx, fontFamily, preferences) => {
    ...
    for (let si = 0; si < (isMultilang ? slideCount : 1); si++) {
      ...
      if (audioData?.wavBuffer) {
        try {
          ...
          segDur = Math.min(Math.max(audioBuf.duration + 0.5, minDur), maxAllowedDur);
          audioPlayed = true;
    ...
    addSystemLog(`Toplam süre: ${totalDuration.toFixed(1)}sn`, 'info');
      ...
      if (ambientTypes.includes(ambientSound)) {
        ...
        try {
          ...
          if (track && track.data) {
            ...
            masterGain = audioCtx.createGain(); masterGain.gain.value = preferences?.backgroundMusicVolume ?? 0.3;
      ...
      for (let frame = 0; frame < totalFrames; frame++) {
        ...
        if (isOutro) {
          ...
          if (!RenderWorkerService._outroParticles || RenderWorkerService._outroParticles.length === 0) {
            ...
            for (let p = 0; p < 20; p++) {
              RenderWorkerService._outroParticles.push({
                ...
                r: 8 + Math.random() * 35, speed: 0.3 + Math.random() * 0.8,
                phase: Math.random() * Math.PI * 2, alpha: 0.05 + Math.random() * 0.15,
          ...
          titleLines.forEach((line, i) => {
            ...
            tg.addColorStop(0, '#FFD700'); tg.addColorStop(0.3, '#FFA500');
            tg.addColorStop(0.7, '#FFD700'); tg.addColorStop(1, '#FFC107');
      ...
      if (this.state.status === 'INIT') {
        if (this.state.config.tip === 'guzel_soz' || this.state.config.tip === 'iddia_analizi') {
          ...
          if (this.state.config.tip === 'iddia_analizi' && this.state.inputType === 'media' && Array.isArray(this.state.inputData)) {
            ...
            if (hasVideo || hasAudio) {
              ...
              this.state.script.videoSlides.unshift({
                topText: 'İŞTE KANIT', spokenText: '', imagePrompts: [], _isRawMedia: true, _rawMediaIndex: 0
              });
          ...
          if (allBasliklar.length >= 1) {
            ...
            } catch (e) { addSystemLog(`Clickbait API hatası: ${e.message}`, 'warn'); }
      ...
      if (this.state.status === 'GENERATING_ASSETS') {
        ...
        if (this.state.script._isGuzelSoz) {
          ...
          if (!this.state.assets.audio[0]) {
            this.state.assets.audio[0] = await MediaSynthesisService.generateAudio(this.state.script.videoSlides[0].spokenText, this.state.preferences.narratorVoice);
          }
          if (this.state.script._isMultilang) {
            for (let mi = 1; mi < slideCount; mi++) {
              if (!this.state.assets.audio[mi]) {
                ...
                this.state.assets.audio[mi] = await MediaSynthesisService.generateAudio(this.state.script.videoSlides[mi].spokenText, this.state.preferences.narratorVoice);
              }
            }
          }
        ...
        if (!this.state.assets.thumbnailAudio) {
          ...
          extraAudioPromises.push(MediaSynthesisService.generateAudio(clickbaitText, this.state.preferences.narratorVoice).then(res => { this.state.assets.thumbnailAudio = res; addSystemLog('Clickbait seslendirme hazır.', 'success'); }));
        }
        if (!this.state.script._isGuzelSoz) {
...
const ImageBitmapCache = {
  ...
  async get(src) {
    ...
    try { const resp = await fetch(src); const blob = await resp.blob(); const bmp = await createImageBitmap(blob); this.cache.set(src, bmp); return bmp; } catch(e) { return null; }
  },
...
function App() {
  ...
  const [gazeteDate, setGazeteDate] = useState(() => { const today = new Date(); const y = today.getFullYear(); const m = String(today.getMonth() + 1).padStart(2, '0'); const d = String(today.getDate()).padStart(2, '0'); return `${y}-${m}-${d}`; });
  ...
  const [config, setConfig] = useState(() => loadSavedJSON('ns_config', {
    duration: '60', aspectRatio: '9:16', videoStyle: 'cinematic', fontStyle: 'modern', imageStyle: 'oil_painting', language: 'tr', subtitles: 'on', resolution: '4K', transition: 'none', outputType: 'video', analysisMode: 'yorumsuz', videoFormat: 'mp4', tip: 'haber', sourceName: '', yorum: ''
  }));
  ...
  const getTargetSeconds = (dur) => { if (dur === 'unlimited') return 0; if (dur === '15') return 15; if (dur === '30') return 30; if (dur === '60') return 60; if (dur === '90') return 90; return 60; };
  ...
  const uploadMediaToCloud = async (blobOrUrl, fileName = 'video.mp4') => {
    try {
      let blob = blobOrUrl;
      if (typeof blobOrUrl === 'string' && blobOrUrl.startsWith('blob:')) { const res = await fetch(blobOrUrl); blob = await res.blob(); }
      ...
      if (!(blob instanceof Blob)) { if (typeof blobOrUrl === 'string' && blobOrUrl.startsWith('http')) return blobOrUrl; return null; }
      const isVideoFile = (blob instanceof Blob && (blob.type.startsWith('video/') || fileName.endsWith('.mp4') || fileName.endsWith('.webm') || fileName.includes('video')));
      if (isVideoFile) {
        try {
          addSystemLog('Instagram Reels uyumluluğu: Sabit 30 FPS CFR MP4 kodlanıyor...', 'info');
          blob = await convertWebMtoMP4(blob, (pct) => { if (pct % 25 === 0) addSystemLog(`Instagram MP4 kodlama: %${pct}`, 'info'); });
          fileName = fileName.replace(/\.[^.]+$/, '') + '.mp4';
          addSystemLog('✓ Sabit 30 FPS MP4 hazır (Instagram Reels & Meta Graph API tam uyumlu).', 'success');
        } catch (convErr) {
          addSystemLog(`MP4 dönüştürme uyarısı: ${convErr.message}`, 'warn');
        }
      }
      ...
      try {
        ...
        const fdProxy = new FormData(); fdProxy.append('file', blob, fileName);
        const rProxy = await fetch('http://localhost:3000/upload_cloud_media', { method: 'POST', body: fdProxy });
        if (rProxy.ok) { const jProxy = await rProxy.json(); if (jProxy.success && jProxy.url) { addSystemLog(`✓ Video/Medya buluta başarıyla yüklendi (${jProxy.provider || 'Python Bridge'}): ${jProxy.url}`, 'success'); return jProxy.url; } }
      ...
      try {
        const fdCat = new FormData(); fdCat.append('reqtype', 'fileupload'); fdCat.append('fileToUpload', blob, fileName);
        const catAttempts = [
          { url: 'https://catbox.moe/user/api.php', label: 'catbox direkt' },
          { url: 'https://api.allorigins.win/raw?url=' + encodeURIComponent('https://catbox.moe/user/api.php'), label: 'catbox allorigins' },
          { url: 'https://corsproxy.io/?url=' + encodeURIComponent('https://catbox.moe/user/api.php'), label: 'catbox corsproxy.io' }
        ];
        for (const attempt of catAttempts) {
          try {
            ...
            if (rCat.ok) { const pubUrl = (await rCat.text()).trim(); if (pubUrl && pubUrl.startsWith('http')) { addSystemLog(`✓ Video/Medya buluta başarıyla yüklendi (catbox kalıcı): ${pubUrl}`, 'success'); return pubUrl; } }
          } catch (pc) {}
        }
        addSystemLog('Catbox.moe yükleme başarısız, alternatif servislere geçiliyor.', 'warn');
      } catch (eCat) { ErrorHandler.silent(eCat); }
      try {
        const fd1 = new FormData(); fd1.append('file', blob, fileName);
        const r1 = await fetch('https://tmpfiles.org/api/v1/upload', { method: 'POST', body: fd1 });
        if (r1.ok) { const j1 = await r1.json(); if (j1.status === 'success' && j1.data?.url) { const directUrl = j1.data.url.replace('https://tmpfiles.org/', 'https://tmpfiles.org/dl/'); addSystemLog(`✓ Video/Medya buluta başarıyla yüklendi (tmpfiles): ${directUrl}`, 'success'); return directUrl; } }
      } catch (e1) { ErrorHandler.silent(e1); }
      try {
        const fd2 = new FormData(); fd2.append('reqtype', 'fileupload'); fd2.append('time', '72h'); fd2.append('fileToUpload', blob, fileName);
        ...
        for (const attempt of litterAttempts) {
          try {
            ...
            if (r2.ok) { const pubUrl = (await r2.text()).trim(); if (pubUrl && pubUrl.startsWith('http')) { addSystemLog(`✓ Video/Medya buluta başarıyla yüklendi (litterbox 72h): ${pubUrl}`, 'success'); return pubUrl; } }
          } catch (ple) {}
        }
        addSystemLog('Litterbox yükleme başarısız.', 'warn');
      } catch (e2) { ErrorHandler.silent(e2); }
      try {
        const fd3 = new FormData(); fd3.append('file', blob, fileName);
        const r3 = await fetch('https://file.io', { method: 'POST', body: fd3 });
        if (r3.ok) { const j3 = await r3.json(); if (j3.success && j3.link) { addSystemLog(`✓ Video/Medya buluta başarıyla yüklendi (file.io): ${j3.link}`, 'success'); return j3.link; } }
      } catch (e3) { ErrorHandler.silent(e3); }
    } catch (e) { addSystemLog('Bulut yükleme uyarısı: ' + e.message, 'warn'); }
    return null;
  };

  // Buffer API ile paylaşım — orijinal korunur (Firebase içermez)
  const shareToBufferAPI = async (text, mediaUrl = null) => {
    ...
    const isHttpsRemoteOrigin = typeof window !== 'undefined' &&
      ...
      (!window.location.hostname || (window.location.hostname.includes('localhost') === false && window.location.hostname.includes('127.0.0.1') === false));
    addSystemLog(`[DEBUG] shareToBufferAPI başladı | token: ${token ? token.slice(0,8)+'...' : 'YOK'}`, 'info');
    if (!token) {
    ...
    if (!directCloudUrl) {
      const activeGazete = gazeteItems && gazeteItems[gazeteCurrentIdx];
      const activeGazeteUrl = activeGazete?.resolvedFull || activeGazete?.resolvedThumb || activeGazete?.rawCandidates?.[0] || activeGazete?.fullSrc || activeGazete?.thumbSrc;
      if (activeGazeteUrl && !activeGazeteUrl.startsWith('blob:') && !activeGazeteUrl.startsWith('data:')) directCloudUrl = activeGazeteUrl;
      else directCloudUrl = 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=1080';
    }
    const serverUrl = await getLinkedInServerUrl();
    if (serverUrl) {
      try {
        const res = await fetch(`${serverUrl}/buffer/share-all`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Local-Proxy-Auth': PROXY_AUTH_TOKEN }, body: JSON.stringify({ text: fullPostText, media_url: directCloudUrl, token }) });
        if (res.ok) { const json = await res.json(); if (json.status === 'success') { addSystemLog(`✓ Buffer üzerinden ${json.channels_count || 3} sosyal medya kanalına başarıyla gönderildi!`, 'success'); return true; } }
      } catch(e) { addSystemLog(`[DEBUG] Sunucu fetch hatası: ${e.message}`, 'warn'); }
    }
    let activeChannels = [];
    try {
      const getChannelsQuery = `query { account { organizations { id } } }`;
      let orgId = null;
      for (const ep of endpoints) {
        try {
          const res = await fetch(ep, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'X-Local-Proxy-Auth': PROXY_AUTH_TOKEN }, body: JSON.stringify({ query: getChannelsQuery, token, variables: {} }) });
          if (res.ok) { const json = await res.json(); orgId = json.data?.account?.organizations?.[0]?.id; if (orgId) break; }
        } catch(e) { addSystemLog(`[DEBUG] Kanal tespiti fetch hatası: ${e.message}`, 'warn'); }
      }
      if (orgId) {
        const chanQuery = `query GetChannels($input: ChannelsInput!) { channels(input: $input) { id name service } }`;
        for (const ep of endpoints) {
          try {
            const res = await fetch(ep, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'X-Local-Proxy-Auth': PROXY_AUTH_TOKEN }, body: JSON.stringify({ query: chanQuery, token, variables: { input: { organizationId: orgId } } }) });
            if (res.ok) { const json = await res.json(); if (Array.isArray(json.data?.channels) && json.data.channels.length > 0) { activeChannels = json.data.channels; break; } }
          } catch(e) { addSystemLog(`[DEBUG] Kanal listesi fetch hatasi: ${e.message}`, 'warn'); }
        }
      }
    } catch(e) { addSystemLog(`[DEBUG] Kanal tespiti genel hata: ${e.message}`, 'warn'); }
    ...
    addSystemLog(`Buffer'a bağlı ${activeChannels.length} sosyal medya kanalı bulundu.`, 'info');
    let successCount = 0;
    for (const ch of activeChannels) {
      const mutation = `mutation CreatePost($input: CreatePostInput!) { createPost(input: $input) { ... on PostActionSuccess { post { id status } } ... on InvalidInputError { message } ... on UnexpectedError { message } ... on LimitReachedError { message } } }`;
      const assetObj = isVideo ? { video: { url: directCloudUrl } } : { image: { url: directCloudUrl } };
      const variables = { input: { channelId: ch.id, text: fullPostText, mode: 'shareNow', schedulingType: 'automatic', needsApproval: false, assets: [assetObj] } };
      if (ch.service === 'instagram') { variables.input.metadata = { instagram: { type: isVideo ? 'reel' : 'post', shouldShareToFeed: true, coverPhotoIndex: 0 } }; if (isVideo && directCloudUrl) variables.input.assets = [{ video: { url: directCloudUrl } }]; }
      else if (ch.service === 'tiktok') variables.input.metadata = { tiktok: { privacy_level: 'PUBLIC_TO_EVERYONE', title: fullPostText.substring(0, 150) } };
      let posted = false;
      for (const ep of endpoints) {
        if (posted) break;
        try {
          const res = await fetch(ep, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'X-Local-Proxy-Auth': PROXY_AUTH_TOKEN }, body: JSON.stringify({ query: mutation, token, variables }) });
          if (res.ok) { const json = await res.json(); if (json.data?.createPost?.post?.id) { posted = true; successCount++; addSystemLog(`✓ ${ch.name} kanalında başarıyla paylaşıldı`, 'success'); } else if (json.data?.createPost?.message) { addSystemLog(`⚠️ ${ch.name} uyarısı: ${json.data.createPost.message}`, 'warn'); } }
        } catch(e) { addSystemLog(`[DEBUG] createPost fetch hatası: ${e.message}`, 'warn'); }
      }
    }
    if (successCount === 0) addSystemLog('⚠️ Buffer paylaşım başarısız.', 'warn');
    return successCount;
  };
  ...
  const shareToSelectedPlatforms = async () => {
    ...
    try {
      ...
      if (count > 0) addSystemLog(`✓ Buffer ile ${count} sosyal medya kanalında paylaşıldı! 🚀`, 'success');
      else addSystemLog('Buffer paylaşımı tamamlandı. ✅', 'success');
    } catch(e) { addSystemLog(`[PAYLAS] Buffer hata: ${e.message}`, 'warn'); }
    try {
      ...
      if (linkedInServerUrl) {
        ...
        if (res && (res.id || res.status === 'success')) addSystemLog(`✓ Doğrudan LinkedIn API ile başarıyla paylaşıldı! 🚀`, 'success');
      } else {
        addSystemLog('LinkedIn sunucusu bulunamadı. Sadece Buffer kullanıldı.', 'info');
      }
    } catch(e) { addSystemLog(`[PAYLAS] LinkedIn hata: ${e.message}`, 'warn'); }
    addSystemLog('[PAYLAS] Paylasim tamamlandi', 'info');
  };

  const shareToPlatform = async (platform, title, videoUrl) => {
    try {
      addSystemLog('Buffer API ile tüm kanallara otomatik gönderiliyor...', 'info');
  ...
  // === YEREL MÜZİK YÜKLEME (IndexedDB) — bulut geri yükleme yok ===
  useEffect(() => {
      ...
      try {
        ...
        if (allMusic.length > 0) {
          ...
          if (!targetBgmId || targetBgmId === 'none') {
            ...
            if (savedPrefs.ambientSound && !['none', 'rain', 'wind', 'waves', 'fire'].includes(savedPrefs.ambientSound)) targetBgmId = savedPrefs.ambientSound;
          }
          let activeTrack = allMusic.find(m => m.id === targetBgmId);
          if (!activeTrack) { activeTrack = allMusic[0]; targetBgmId = activeTrack.id; }
          if (activeTrack && activeTrack.data) {
            ...
            setPrefs(p => { const np = { ...p, ambientSound: targetBgmId, customBgMusicName: activeTrack.name, customBgMusicId: targetBgmId }; SafeStorage.setItem('ns_prefs', JSON.stringify(np)); return np; });
            addSystemLog(`✓ Seçili müzik yüklendi: ${activeTrack.name}`, 'info');
          }
        } else {
          // Sessiz mod: Müzik kütüphanesi boş uyarısı kaldırıldı
        }
  ...
  useEffect(() => {
    const preloadLocal = async () => {
      try {
        ...
        setStudioMedia(s => ({ ...s, outroUrl: s.outroUrl || localOutro, musicList: s.musicList.length > 0 ? s.musicList : allMusics, customSceneImages: csi, isLoading: false, statusMsg: 'Yerel Mod', syncedFolderName: savedFolderName }));
      } catch (e) {
  ...
  const handleFolderSelect = async () => { if (musicFileInputRef.current) musicFileInputRef.current.click(); };
  ...
  const playMusicPreview = (url) => {
    try {
      ...
      const audio = new Audio(url); audio.volume = prefs.backgroundMusicVolume ?? 0.3;
  ...
  const handleFolderMusicSelect = useCallback(async (musicId) => {
    ...
    setPrefs(p => { const np = { ...p, ambientSound: musicId, customBgMusicName: track.name, customBgMusicId: musicId }; SafeStorage.setItem('ns_prefs', JSON.stringify(np)); return np; });
  ...
  const GAZETE_CACHE_KEY = 'ns_gazeteResolved_v6';
  ...
  const isLikelyGazeteImage = (url = '') => {
    ...
    return /img\.aydinlik\.com\.tr|img\.piri\.net|[si]\.gazeteoku\.com|storage\/newspapers|gazete.*(?:image|upload|manset)/i.test(url);
  };
  ...
  const dateBackList = (date, count = 8) => {
    const base = new Date(`${date}T12:00:00`); const result = [];
    for (let i = 0; i < count; i++) { const d = new Date(base); d.setDate(base.getDate() - i); result.push(d.toISOString().split('T')[0]); }
    return result;
  };
  const uniqueGazeteUrls = (urls = []) => urls.filter(Boolean).filter((url, index, list) => list.indexOf(url) === index);
  const weservGazeteUrl = (url, width = 900, quality = 88) => url && !url.startsWith('data:') ? `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=${width}&fit=contain&output=jpg&q=${quality}` : url;
  const createGazeteVariants = (rawUrls = [], cached = null) => {
    ...
    const fullCandidates = uniqueGazeteUrls([cached?.full, ...cleanedRaw.map(url => weservGazeteUrl(url, 1000, 90)), ...cleanedRaw]);
    const thumbCandidates = uniqueGazeteUrls([cached?.thumb, ...cleanedRaw.map(url => weservGazeteUrl(url, 360, 76)), ...fullCandidates]);
  ...
  const buildGazeteCards = (date) => {
    const cache = getGazeteCache(); const days = dateBackList(date, 8);
    return GAZETE_MASTER_LIST.map(name => {
      const meta = GAZETE_META[name] || {};
      const rawUrls = [];
      if (meta.ayd) { for (const day of days) rawUrls.push(`https://img.aydinlik.com.tr/rcman/Cw1200h2010q95gc/storage/newspapers/${day}/${meta.ayd}.jpg`); }
      if (name === 'Gazete Pencere') { for (const day of days) { const [year, month, dayNum] = day.split('-'); rawUrls.push(`https://cdn.gazetepencere.com/other/${year}/${month}/${dayNum}/dddd.jpg`); } }
      const cached = cache[`${date}|${name}`] || null;
      const variants = createGazeteVariants(rawUrls, cached);
      const placeholder = makeGazetePlaceholder(name);
      return { name, ...variants, fullCandidates: [...variants.fullCandidates, placeholder], thumbCandidates: [...variants.thumbCandidates, placeholder], fullSrc: variants.fullCandidates[0] || placeholder, thumbSrc: variants.thumbCandidates[0] || placeholder, resolvedFull: cached?.full || '', resolvedThumb: cached?.thumb || '', sources: meta.ayd ? ['Aydınlık CDN'] : [], loaded: false, placeholder, revision: 0 };
    });
  };
  ...
  const fetchGazeteSourceTextsFast = async (pageUrl, signal) => {
    ...
    const jobs = proxyUrls.map(async proxyUrl => {
      const response = await fetchWithGazeteTimeout(proxyUrl, signal, 6500);
      if (!response.ok) throw new Error(String(response.status));
      const contentType = (response.headers.get('content-type') || '').toLowerCase();
      if (contentType.includes('json')) { const json = await response.json(); const body = json?.contents || json?.data || json?.body || ''; if (typeof body !== 'string' || body.length < 200) throw new Error('empty'); return body; }
      const body = await response.text(); if (body.length < 200) throw new Error('empty'); return body;
    });
    const settled = await Promise.allSettled(jobs);
    return uniqueGazeteUrls(settled.filter(x => x.status === 'fulfilled').map(x => x.value));
  };
  const extractGazeteCandidates = (payload, pageUrl, sourceLabel, forcedName = '') => {
    ...
    const add = (url, meta = {}) => {
      ...
      if (forcedName && contextName && contextName !== forcedName) return;
      seen.add(src);
      const contextBoost = forcedName && contextName === forcedName ? 45 : 0;
      out.push({ name: forcedName || meta.name || '', src, source: sourceLabel, alt: meta.alt || '', title: meta.title || '', hint: meta.hint || '', score: (meta.score || 10) + contextBoost });
    };
    try {
      const doc = new DOMParser().parseFromString(text, 'text/html');
      doc.querySelectorAll('meta[property="og:image"],meta[name="twitter:image"],meta[property="twitter:image"]').forEach(el => add(el.getAttribute('content'), { hint: doc.title, score: 70 }));
    ...
    while ((match = metaRegex.exec(text))) add(match[1] || match[2], { score: 70 });
  ...
  const extractGztDetailUrls = (payload, slug, selectedDate) => {
    const text = decodeGazeteUrl(payload); const urls = [];
    const absolute = new RegExp(`https?:\\/\\/gazetemanset\\.gzt\\.com\\/${escapeGazeteRegExp(slug)}\\/\\d{1,2}-\\d{1,2}-\\d{4}`, 'gi');
    const relative = new RegExp(`\\/${escapeGazeteRegExp(slug)}\\/\\d{1,2}-\\d{1,2}-\\d{4}`, 'gi');
    for (const found of text.match(absolute) || []) if (!urls.includes(found)) urls.push(found);
    for (const found of text.match(relative) || []) { const full = `https://gazetemanset.gzt.com${found}`; if (!urls.includes(full)) urls.push(full); }
    for (const day of dateBackList(selectedDate, 12)) { const [year, month, dayNum] = day.split('-'); const full = `https://gazetemanset.gzt.com/${slug}/${dayNum}-${month}-${year}`; if (!urls.includes(full)) urls.push(full); }
    return urls.slice(0, 6);
  };
  const fetchIndividualGazeteCandidates = async (name, selectedDate, signal) => {
    const meta = GAZETE_META[name] || {}; const candidates = []; const jobs = [];
    if (meta.go) jobs.push((async () => { const url = `https://www.gazeteoku.com/gazeteler/${meta.go}`; const texts = await fetchGazeteSourceTextsFast(url, signal); for (const body of texts) candidates.push(...extractGazeteCandidates(body, url, 'GazeteOku', name)); })());
    if (meta.ayd) jobs.push((async () => { const url = `https://www.aydinlik.com.tr/gazete-mansetleri/${meta.ayd}`; const texts = await fetchGazeteSourceTextsFast(url, signal); for (const body of texts) candidates.push(...extractGazeteCandidates(body, url, 'Aydınlık', name)); })());
    if (meta.gzt) jobs.push((async () => {
      const indexUrl = `https://gazetemanset.gzt.com/${meta.gzt}`;
      const indexTexts = await fetchGazeteSourceTextsFast(indexUrl, signal);
      const details = [];
      for (const body of indexTexts) { candidates.push(...extractGazeteCandidates(body, indexUrl, 'GZT', name)); for (const detail of extractGztDetailUrls(body, meta.gzt, selectedDate)) if (!details.includes(detail)) details.push(detail); }
      if (candidates.some(x => x.source === 'GZT')) return;
      const detailCandidates = uniqueGazeteUrls([...details, ...extractGztDetailUrls('', meta.gzt, selectedDate)]).slice(0, 4);
      const detailJobs = detailCandidates.map(async detailUrl => { const texts = await fetchGazeteSourceTextsFast(detailUrl, signal); for (const body of texts) candidates.push(...extractGazeteCandidates(body, detailUrl, 'GZT', name)); });
      await Promise.allSettled(detailJobs);
    })());
    await Promise.allSettled(jobs);
    const seen = new Set();
    return candidates.filter(candidate => { if (!candidate.src || seen.has(candidate.src)) return false; seen.add(candidate.src); return true; }).sort((a, b) => (b.score || 0) - (a.score || 0));
  };
  const prependLiveGazeteCandidates = (item, candidates) => {
    ...
    return { ...item, ...variants, fullCandidates: [...variants.fullCandidates, item.placeholder], thumbCandidates: [...variants.thumbCandidates, item.placeholder], fullSrc: variants.fullCandidates[0] || item.fullSrc, thumbSrc: variants.thumbCandidates[0] || item.thumbSrc, sources, revision: (item.revision || 0) + 1 };
  };
  ...
  const fetchGazeteManşetleri = async () => {
    ...
    setGazeteLoading(true); setGazeteError(''); setGazeteCurrentIdx(0);
    ...
    try {
      ...
      addSystemLog(`✓ ${cards.length}/30 gazete kartı anında oluşturuldu.`, 'success');
      const liveNames = ['Hürriyet', 'Milliyet', 'Yeni Mesaj'];
      const results = await Promise.allSettled(liveNames.map(async name => ({ name, candidates: await fetchIndividualGazeteCandidates(name, selectedDate, controller.signal) })));
      if (requestId !== gazeteRequestIdRef.current || controller.signal.aborted) return;
      const found = new Map();
      for (const result of results) if (result.status === 'fulfilled' && result.value.candidates.length) found.set(result.value.name, result.value.candidates);
      if (found.size) { setGazeteItems(prev => prev.map(item => found.has(item.name) ? prependLiveGazeteCandidates(item, found.get(item.name)) : item)); addSystemLog(`✓ Canlı kaynak çözümü: ${[...found.keys()].join(', ')}`, 'success'); }
      const missing = liveNames.filter(name => !found.has(name));
      if (missing.length) addSystemLog(`Canlı kaynak bekleyenler: ${missing.join(', ')}. Kart açıldığında alternatifler tekrar denenecek.`, 'warn');
    } catch (e) { if (e?.name !== 'AbortError') { setGazeteError('Gazete manşetleri hazırlanırken hata oluştu.'); addSystemLog('Gazete yükleme hatası: ' + (e?.message || e), 'error'); } }
    finally { if (requestId === gazeteRequestIdRef.current) setGazeteLoading(false); }
  };
  const handleGazeteImageError = (event, item, kind = 'thumb') => {
    ...
    if (image.dataset.gazeteRescue !== '1') { image.dataset.gazeteRescue = '1'; rescueGazeteItem(item?.name); }
  };
  const handleGazeteImageLoad = (event, item, kind = 'thumb') => {
    const image = event.currentTarget;
    const current = image.currentSrc || image.src;
    image.style.opacity = '1';
    if (!item || !current || current.startsWith('data:image/')) return;
    const cacheField = kind === 'full' ? 'full' : 'thumb';
    persistGazeteResolution(gazeteDate, item.name, cacheField, current);
    item.loaded = true;
    if (kind === 'full') { item.resolvedFull = current; item.fullSrc = current; }
    else { item.resolvedThumb = current; item.thumbSrc = current; }
  };
  ...
  const addFullImageToMedia = async (src, name) => {
    try {
      ...
      try {
        ...
        dataUrl = await new Promise((resolve, reject) => {
          img.onload = () => { try { const c = document.createElement('canvas'); c.width = img.naturalWidth; c.height = img.naturalHeight; c.getContext('2d').drawImage(img, 0, 0); resolve(c.toDataURL('image/jpeg', 0.92)); } catch(e) { reject(e); } };
      ...
      if (!dataUrl) {
        for (const proxy of CORS_PROXIES) {
          try {
            ...
            const r = await fetch(proxyUrl); if (!r.ok) continue;
            let blob;
            if (proxy.json) { const j = await r.json(); const innerUrl = j.contents || j.data; if (!innerUrl) continue; blob = await (await fetch(innerUrl)).blob(); }
    ...
    } catch (e) { addSystemLog('Aktarma hatası: ' + e.message, 'error'); }
    finally { setGazeteLoading(false); }
  };
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {showApiKeyModal && (
            <div className="mb-4 bg-slate-900/90 border border-indigo-500/30 p-4 rounded-2xl shadow-xl flex flex-col md:flex-row items-center gap-3 backdrop-blur-md">
              <div className="flex-1 w-full">
                ...
                <input type="password" value={userApiKey} onChange={(e) => setUserApiKeyState(e.target.value)} placeholder="AI Studio Gemini API Key yapıştırın (AIzaSy...)" className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-indigo-500 font-mono" />
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-3 md:p-4 shadow-2xl relative z-10 mb-4">
            ...
            <div className="bg-black/30 p-2.5 rounded-xl border border-slate-800 flex items-center gap-2 mb-3">
              <div className="flex items-center gap-2 w-full">
                <input type="password" defaultValue={SafeStorage.getItem('BUFFER_API_KEY') || ''} onChange={(e) => { SafeStorage.setItem('BUFFER_API_KEY', e.target.value); }} placeholder="Buffer API Token (TzKT...)" className="w-full bg-transparent text-sm text-slate-200 outline-none placeholder:text-slate-600" />
                ...
                <div className="flex items-center justify-between gap-2 mb-3 flex-wrap bg-slate-950/40 p-2.5 rounded-xl border border-slate-800">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1.5 bg-slate-800/80 border border-slate-700 rounded-lg px-2 py-1">
                      ...
                      <input type="date" value={gazeteDate} max={new Date().toISOString().split('T')[0]} onChange={(e) => { setGazeteDate(e.target.value); }} className="bg-transparent text-slate-200 text-[10px] font-bold border-none outline-none cursor-pointer" />
                    </div>
                    ...
                    {gazeteGalleryView === 'grid' ? (
                      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 max-h-[50vh] overflow-y-auto p-1">
                        {gazeteItems.map((item, idx) => (
                          <div key={`${item.name}-${item.revision || 0}`} className="group relative bg-slate-800/50 rounded-xl overflow-hidden border border-slate-700/50 hover:border-emerald-500/50 transition-all cursor-pointer"
                            ...
                            <img src={item.resolvedThumb || item.thumbSrc} className="w-full aspect-[800/1340] object-contain block bg-slate-950" alt={item.name} loading="lazy" decoding="async" referrerPolicy="no-referrer" onLoad={(e) => handleGazeteImageLoad(e, item, 'thumb')} onError={(e) => handleGazeteImageError(e, item, 'thumb')} />
                      ...
                      <div className="relative">
                        ...
                        <div className="relative bg-black/50 rounded-xl overflow-hidden border border-slate-700/50">
                          <img key={`${gazeteItems[gazeteCurrentIdx]?.name}-${gazeteItems[gazeteCurrentIdx]?.revision || 0}`} src={gazeteItems[gazeteCurrentIdx]?.resolvedFull || gazeteItems[gazeteCurrentIdx]?.fullSrc} className="w-full h-auto block bg-slate-950" alt={gazeteItems[gazeteCurrentIdx]?.name} decoding="async" referrerPolicy="no-referrer" onLoad={(e) => handleGazeteImageLoad(e, gazeteItems[gazeteCurrentIdx], 'full')} onError={(e) => handleGazeteImageError(e, gazeteItems[gazeteCurrentIdx], 'full')} />
                        </div>
                      </div>
                    )}
                  </div>
...
// ── App Export & Global Scope Attachment ──────────────────────────────────────
