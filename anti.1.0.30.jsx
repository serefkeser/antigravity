...
function App() {
  ...
  const uploadMediaToCloud = async (blobOrUrl, fileName = 'video.mp4') => {
    try {
      ...
      if (blob instanceof Blob && (blob.type.includes('webm') || fileName.endsWith('.webm'))) {
        try {
          addSystemLog('Instagram uyumluluğu: WebM → MP4 dönüştürülüyor...', 'info');
          blob = await convertWebMtoMP4(blob, (pct) => { if (pct % 25 === 0) addSystemLog('MP4 dönüştürme: %' + pct, 'info'); });
          fileName = fileName.replace(/\.webm$/i, '.mp4');
          addSystemLog('MP4 dönüştürme tamamlandı (Instagram uyumlu).', 'success');
        } catch (convErr) { addSystemLog('MP4 dönüştürme başarısız, webm olarak devam: ' + convErr.message, 'warn'); }
      }
